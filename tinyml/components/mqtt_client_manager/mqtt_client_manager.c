...
{
    if (!s_connected) {
        ESP_LOGW(TAG, "Sin conexión MQTT — descartando evento drinking (ventana #%lu)",
                 window_count);
        return ESP_ERR_INVALID_STATE;
    }
    ...
    /* Construir topic */
    char topic[64];
    snprintf(topic, sizeof(topic), "sensors/%s/drinking", MQTT_DEVICE_ID);
    ...
    cJSON_AddStringToObject(root, "device_id", MQTT_DEVICE_ID);
    cJSON_AddStringToObject(root, "label",     "drinking");

    time_t now;
    time(&now);
    cJSON_AddNumberToObject(root, "timestamp",   (double)now);
    cJSON_AddNumberToObject(root, "temperature",
                            (double)((int)(temperature * 10)) / 10.0);
    cJSON_AddNumberToObject(root, "window",      (double)window_count);
    ...
    int msg_id = esp_mqtt_client_publish(
        s_client, topic, json_str, 0, MQTT_PUBLISH_QOS, 0);

    free(json_str);

    if (msg_id < 0) {
        ESP_LOGE(TAG, "Error publicando evento drinking");
        return ESP_FAIL;
    }

    s_publish_count++;
    ESP_LOGI(TAG, "Evento drinking publicado → %s (ventana #%lu, temp=%.1f°C)",
             topic, window_count, temperature);
